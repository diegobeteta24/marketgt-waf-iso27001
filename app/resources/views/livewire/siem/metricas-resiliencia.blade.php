...
<div wire:poll.60s class="space-y-6">
    <div class="rounded-xl border border-zinc-200 bg-white p-4 dark:border-zinc-700 dark:bg-zinc-900">
        <div class="flex flex-wrap items-start justify-between gap-6">
            ...
            <svg viewBox="0 0 260 180" class="h-40 w-auto" role="img" aria-label="Estado de los tres vertices de la ciberresiliencia">
                <polygon
                    ...
                    @foreach ($vertice['metricas'] as $metrica)
                        <div class="space-y-2 p-4">
                            <div class="flex items-start justify-between gap-3">
                                <p class="text-sm font-medium text-zinc-900 dark:text-white">{{ $metrica['nombre'] }}</p>
                            ...
                            <p @class([
                                'siem-numero text-2xl font-semibold',
                                'text-zinc-900 dark:text-white' => $metrica['estado'] !== CalculadoraMetricas::SIN_DATOS,
                                'text-zinc-400 dark:text-zinc-500 text-base font-normal italic' => $metrica['estado'] === CalculadoraMetricas::SIN_DATOS,
                            ...
                            <p class="text-xs leading-relaxed text-zinc-500 dark:text-zinc-400">{{ $metrica['origen'] }}</p>

                            @if ($metrica['advertencia'])
                                <p class="text-xs font-medium text-amber-600 dark:text-amber-400">{{ $metrica['advertencia'] }}</p>
                            @endif
                        </div>
                    @endforeach
                </div>
